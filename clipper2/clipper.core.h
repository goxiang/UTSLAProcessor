...
namespace Clipper2Lib
{
...
#else
    std::transform(path.begin(), path.end(), my_back_inserter(result),
      [scale_x, scale_y](const auto& pt) 
    ...
    double scale_x, double scale_y, int& error_code)
  {
    ...
    std::transform(paths.begin(), paths.end(), my_back_inserter(result),
  ...
  inline Path<T1> TransformPath(const Path<T2>& path)
  {
    Path<T1> result;
    result.reserve(path.size());
    std::transform(path.cbegin(), path.cend(), my_back_inserter(result),
  ...
  {
    Paths<T1> result;
    std::transform(paths.cbegin(), paths.cend(), my_back_inserter(result),
